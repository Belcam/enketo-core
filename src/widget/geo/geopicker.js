...
console.log(map, index, map.tiles === 'string', /^GOOGLE_(SATELLITE|ROADMAP|HYBRID|TERRAIN)/.test( map.tiles ), /^BING_(Aerial|AerialWithLabels|Birdseye|BirdseyeWithLabels|Road)/.test( map.tiles ));
            if ( typeof map.tiles === 'string') {
              ...
              } else if( /^BING_(Aerial|AerialWithLabels|Birdseye|BirdseyeWithLabels|Road)/.test( map.tiles ) ) {
                tasks.push( that._getBingTileLayer( map, index ) );
              }
            } else
            if ( map.tiles ) {
              if( map.type == 'WMS' )
                tasks.push( that._getLeafletTileWMSLayer( map, index ) );
              else
    ...
    /**
     * Asynchronously (fake) obtains a Leaflet/Mapbox tilelayerWMS
     *
     * @param  {{}}     map   map layer as defined in the apps configuration
     * @param  {number} index the index of the layer
     * @return {Promise}
     */
    Geopicker.prototype._getLeafletTileWMSLayer = function( map, index ) {
        var options = this._getTileOptions( map, index );
        options.format = map.format || 'image/png';
        options.transparent = map.transparent || true;
        options.layers = map.tiles.join(',');
        return Promise.resolve( L.tileLayer.wms( map.url, options ) );
    };
    ...
    /**
     * Asynchronously obtains a Bing tilelayer
     *
     * @param  {{}}     map   map layer as defined in the apps configuration
     * @param  {number} index the index of the layer
     * @return {Promise}
     */
    Geopicker.prototype._getBingTileLayer = function( map, index ) {
        var options = this._getTileOptions( map, index ),
            key = map.key;
        options.type = map.tiles.substring( 5 );
console.log(options, key);
        return Promise.resolve( L.BingLayer( key, options ) );
    };
...
/*
    ...
    /*
     * Bing layers
     * from https://github.com/shramov/leaflet-plugins/blob/master/layer/tile/Bing.js
     */ 
